...
class SudokuApp:
    ...
    def menu_screen(self):
        ...
        self.menu_label = tk.Label(self.master, text="Welcome to the Mystery Grid: The Case of Missing Numbers", font=("Helvetica", 20))
        self.menu_label.pack(pady=20)

        self.play_button = tk.Button(self.master, text="Play the Mystery Grid", font=("Helvetica", 20), command=self.sudoku_screen, width=20, height=2)
...
class SudokuGUI:
    def __init__(self, master):
        ...
        self.cell_size = 100  # Adjust the cell size as needed

        self.cells = [[tk.Entry(self.canvas, width=2, justify='center', font=("Arial", 24), borderwidth=2)
                       for _ in range(9)] for _ in range(9)]
        for i in range(9):
            for j in range(9):
                self.canvas.create_window(self.cell_size * j + self.cell_size // 2, self.cell_size * i + self.cell_size // 2, window=self.cells[i][j])

        for i in range(9):
            for j in range(9):
                self.canvas.create_window(50 * j + 25, 50 * i + 25, window=self.cells[i][j])

        self.solve_button = tk.Button(master, text="Solve", command=self.solve, font=("Helvetica", 15), background="salmon4", pady=5, padx=10)
        self.canvas.create_window(75, 500, window=self.solve_button)

        self.clear_button = tk.Button(master, text="Clear", command=self.clear, font=("Helvetica", 15), background="salmon4", pady=5, padx=10)
        self.canvas.create_window(175, 500, window=self.clear_button)

        self.generate_button = tk.Button(master, text="Generate", command=self.generate, font=("Helvetica", 15), background="salmon4", pady=5, padx=10)
        self.canvas.create_window(300, 500, window=self.generate_button)

        self.check_button = tk.Button(master, text="Check", command=self.check, font=("Helvetica", 15), background="salmon4", pady=5, padx=10  )
        self.canvas.create_window(425, 500, window=self.check_button)
